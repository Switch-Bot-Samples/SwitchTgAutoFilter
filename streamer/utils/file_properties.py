...
async def parse_file_id(message: "Message", thumb) -> Optional[FileId]:
    media = get_media_from_message(message, thumb)
    if media:
        return FileId.decode(media.file_id)


async def parse_file_unique_id(message: "Messages", thumb) -> Optional[str]:
    media = get_media_from_message(message, thumb)
    if media:
...
async def get_file_ids(
    client: Client, chat_id: int, message_id: int, thumb: bool = False
) -> Optional[FileId]:
    message = await client.get_messages(chat_id, message_id)
    if message.empty:
        raise FIleNotFound
    media = get_media_from_message(message, thumb)
    file_unique_id = await parse_file_unique_id(message, thumb)
    file_id = await parse_file_id(message, thumb)
...
def get_media_from_message(message: "Message", thumb: bool = False) -> Any:
    media_types = (
        "audio",
        "document",
        "photo",
    ...
    for attr in media_types:
        ...
        if media:
            if thumb and (thumb_ := getattr(media, "thumbs", None)):
                return thumb_[0]
            return media
