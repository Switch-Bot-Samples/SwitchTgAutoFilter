...
from .filters_mdb import run_async


@run_async
def add_connection(group_id, user_id):
...
@run_async
def if_active(user_id, group_id):
    query = mycol.find_one(
        {"_id": user_id},
        {"_id": 0, "group_details": 0}
    )
...
@run_async
def make_active(user_id, group_id):
...
@run_async
def delete_connection(user_id, group_id):
