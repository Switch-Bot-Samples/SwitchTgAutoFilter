...
async def start(client: Client, message: Message):
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton(
                    "ℹ️ Hᴇʟᴩ ℹ️", url=f"https://t.me/{temp.U_NAME}?start=help"
                )
            ],
        ]
        await message.reply(
    ...
    if newUser:
        ...
        try:
            await message.reply_cached_media(
                ...
                caption="How to search and download movies?",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "Report Issue", url="https://t.me/trishaAdmin_bot"
                            )
                        ]
                    ]
                ),
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append(
                    [
                        InlineKeyboardButton(
                            "⟳ Tʀʏ Aɢᴀɪɴ", callback_data=f"{pre}#{file_id}"
                        )
                    ]
                )
    ...
    if file_id.isdigit():
        ...
        await client.send_message(
            chat_id=message.from_user.id,
            text=f"**{get_size(file.file_size)} | [{file.description}]({verifyLink})**\n\nClick below url to get your movie!",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
        ...
        if newUser:
            await client.send_message(
                LOG_CHANNEL,
                f"""{message.from_user.mention} #SearchedFor {file.description} | {get_size(file.file_size)} |[{file_id}]""",
            )
        return
...
async def settings(client, message):
    ...
    if not userid:
        return await message.reply(
            f"Yᴏᴜ Aʀᴇ Aɴᴏɴʏᴍᴏᴜs Aᴅᴍɪɴ. Usᴇ /connect {message.chat.id} Iɴ PM"
        )
        ...
        if grpid is not None:
            ...
            try:
                ...
                return await message.reply_text(
                    "Mᴀᴋᴇ Sᴜʀᴇ I'ᴍ Pʀᴇsᴇɴᴛ Iɴ Yᴏᴜʀ Gʀᴏᴜᴘ!!", quote=True
                )
        else:
            return await message.reply_text(
                "I'ᴍ Nᴏᴛ Cᴏɴɴᴇᴄᴛᴇᴅ Tᴏ Aɴʏ Gʀᴏᴜᴘs!", quote=True
            )
