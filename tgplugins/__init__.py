...
from tgconfig import SUPPORT_CHAT
...
async def banned_users(_, client, message: Message):
    return (message.from_user is not None or not message.sender_chat) and (
...
async def ban_reply(bot, message):
    ...
    await message.reply(
        f"Sᴏʀʀʏ Dᴜᴅᴇ, Yᴏᴜ Aʀᴇ Bᴀɴɴᴇᴅ Tᴏ Usᴇ Mᴇ. \nBᴀɴ Rᴇᴀsᴏɴ: {ban['ban_reason']}"
    )
