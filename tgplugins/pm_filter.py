...
from tgconfig import AUTH_USERS, PM_IMDB, SINGLE_BUTTON, PROTECT_CONTENT, SPELL_CHECK_REPLY, IMDB_TEMPLATE, IMDB_DELET_TIME, PMFILTER, G_FILTER, SHORT_URL, SHORT_API
...
async def pm_next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {getattr(file, 'description', file.file_name)}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
        else:
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {getattr(file, 'description', file.file_name)}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
        else:
