...
import logging, re, asyncio, os, sys
...
async def savegroup_and_welcome(bot, message):
    ...
    if bot.id in r_j_check:
        if not await db.get_chat(message.chat.id):
            ...
            await db.add_chat(
                message.chat.id, message.chat.title, message.chat.username
            )
        ...
        await message.reply(
            text="❤️ Tʜᴀɴᴋs Tᴏ Aᴅᴅ Mᴇ Tᴏ Yᴏᴜ'ʀ Gʀᴏᴜᴘ.\n» Dᴏɴ'ᴛ Fᴏʀɢᴇᴛ Tᴏ Mᴀᴋᴇ Mᴇ Aᴅᴍɪɴ.\n» Is Aɴʏ Dᴏᴜʙᴛ's Aʙᴏᴜᴛ Usɪɴɢ Mᴇ Cʟɪᴄᴋ Bᴇʟᴏᴡ Bᴜᴛᴛᴏɴ...✨",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
        for u in message.new_chat_members:
            ...
            if WELCOM_PIC:
                ...
                temp.MELCOW["welcome"] = await message.reply_text(
                    text=WELCOM_TEXT.format(user=u.mention, chat=message.chat.title)
                )
...
async def disable_chat(bot, message):
    ...
    if cha_t["is_disabled"]:
        return await message.reply(
            f"Tʜɪꜱ Cʜᴀᴛ Is Aʟʀᴇᴅʏ  Dɪꜱᴀʙʟᴇᴅ:\nRᴇᴀꜱᴏɴ: <code> {cha_t['reason']} </code>"
        )
...
async def gen_invite(bot, message):
    ...
    try:
        ...
        return await message.reply(
            "Iɴᴠɪᴛᴇ Lɪɴᴋ Gᴇɴᴇʀᴀᴛɪᴏɴ Fᴀɪʟᴇᴅ, Iᴀᴍ Nᴏᴛ Hᴀᴠɪɴɢ Sᴜғғɪᴄɪᴇɴᴛ Rɪɢʜᴛs"
        )
    except Exception as e:
...
async def ban_a_user(bot, message):
    ...
    try:
        ...
        return await message.reply(
            "Tʜɪs Is Aɴ Iɴᴠᴀʟɪᴅ Usᴇʀ, Mᴀᴋᴇ Sᴜʀᴇ Iᴀ Hᴀᴠᴇ Mᴇᴛ Hɪᴍ Bᴇғᴏʀᴇ"
        )
    except IndexError:
...
async def user_info(client, message):
    ...
    if message.chat.type in ((enums.ChatType.SUPERGROUP, enums.ChatType.CHANNEL)):
        try:
            ...
            message_out_str += (
                f"<b>➲ᴊᴏɪɴᴇᴅ ᴛʜɪꜱ ᴄʜᴀᴛ ᴏɴ:</b> <code>{joined_date}</code>\n"
            )
...
async def imdb_search(client, message):
    if " " in message.text:
        ...
        await k.edit(
            "Hᴇʀᴇ Is Wʜᴀᴛ I Fᴏᴜɴᴅ Oɴ Iᴍᴅʙ", reply_markup=InlineKeyboardMarkup(btn)
        )
    else:
