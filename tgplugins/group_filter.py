...
from tgconfig import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, PM_IMDB, SINGLE_BUTTON, PROTECT_CONTENT, \
...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {getattr(file, 'description', '') or file.file_name}", callback_data=f'files#{nxreq}#{file.file_id}')] for file in files ]
        else:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {getattr(file, 'description', file.file_name)}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ]
        else:
