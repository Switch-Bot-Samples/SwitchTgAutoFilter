    ...
    if (len(extracted) >= 2) and not message.reply_to_message:
        ...
        try:
            ...
            reply_text, btn, alert = (
                parser(extracted[1], text, "alertmessage")
                if message.reply_to_message.sticker
                else parser(message.reply_to_message.caption.html, text, "alertmessage")
            )
        except:
            reply_text = ""
            btn = "[]"
        ...
        try:
            ...
            reply_text, btn, alert = parser(
                message.reply_to_message.text.html, text, "alertmessage"
            )
        except:
            reply_text = ""
            btn = "[]"
...
async def addgfilter(client, message):
    ...
    if (len(extracted) >= 2) and not message.reply_to_message:
        ...
        try:
            ...
            fileid = msg.file_id if msg else None
            reply_text, btn, alert = (
                parser(extracted[1], text, "galert")
                if message.reply_to_message.sticker
                else parser(message.reply_to_message.caption.html, text, "galert")
            )
        except:
            reply_text = ""
            btn = "[]"
        ...
        try:
            fileid = None
            reply_text, btn, alert = parser(
                message.reply_to_message.text.html, text, "galert"
            )
        except:
            reply_text = ""
            btn = "[]"
