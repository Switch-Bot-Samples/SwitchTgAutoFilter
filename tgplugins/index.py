...
@Client.on_message(
    filters.chat(CHANNELS) & (filters.document | filters.video | filters.audio)
)
...
async def index_files(bot, query):
    ...
    if lock.locked():
        return await query.answer(
            "Wᴀɪᴛ Uɴᴛɪʟ Pʀᴇᴠɪᴏᴜs Pʀᴏᴄᴇss Cᴏᴍᴘʟᴇᴛᴇ", show_alert=True
        )
    msg = query.message
        ...
        [
            [
                InlineKeyboardButton(
                    "✨ ʏᴇꜱ", callback_data=f"index#{chat_id}#{last_msg_id}"
                )
            ],
